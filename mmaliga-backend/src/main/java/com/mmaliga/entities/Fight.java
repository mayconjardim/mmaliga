...
public class Fight implements Serializable {
	...
	public void actClinchTakedown(Fighter act, Fighter pas) {
		...
		if (takedownType(act) == 1) {
			switch (attackLevel) {
			case 1:
				generateComment(Comments.judoTD1);
				break;
			case 2:
				generateComment(Comments.judoTD1);
				break;
			...
			switch (attackLevel) {
			case 1:
				generateComment(Comments.wrestlingTD1);
				break;
			case 2:
				generateComment(Comments.wrestlingTD1);
				break;
		...
		// bout.updateStatistic(getFighterNumber(act), ST_TAKEDOWNS,
		// extractHitsLaunched(fullComment), 0);
	...
	public void actGnP(Fighter act, Fighter pas) {
		...
		// bout.updateStatistic(getFighterNumber(act), StatisticType.GNP,
		// extractHitsLaunched(fullComment), 0);
		...
		// Bout.UpdateStatistic(GetFighterNumber(act), StatisticType.stGnP, 0,
		// ExtractHitsLanded(FullComment));
	...
	public void actPositioning(Fighter act, Fighter pas) {
		...
		case 0:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				...
				case -99:
				case 0:
					generateComment(Comments.inRearMountMoves);
					break;
				case 1:
					generateComment(Comments.inRearMountOneHookMoves);
					break;
				default:
					generateComment(Comments.inRearMountTwoHooksMoves);
					break;
				}
			} else {
				switch (numHooks) {
				case -99:
				case 0:
					generateComment(Comments.defInRearMountMoves);
					break;
				case 1:
					generateComment(Comments.defInRearMountOneHookMoves);
					break;
				default:
					generateComment(Comments.defInRearMountTwoHooksMoves);
					break;
				}
			}
			break;
		case 1:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				generateComment(Comments.inFullMountMoves);
			} else {
				generateComment(Comments.defInFullMountMoves);
			}
			break;
		case 2:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				generateComment(Comments.inSideMountMoves);
			} else {
				generateComment(Comments.defInSideMountMoves);
			}
			break;
		case 3:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				generateComment(Comments.inHalfGuardMoves);
			} else {
				generateComment(Comments.defInHalfGuardMoves);
			}
			break;
		case 4:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				generateComment(Comments.inOpenGuardMoves);
			} else {
				generateComment(Comments.defInOpenGuardMoves);
			}
			break;
		case 5:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				generateComment(Comments.inClosedGuardMoves);
			} else {
				generateComment(Comments.defInClosedGuardMoves);
			}
			break;
		case 6:
			if (fighterActiveOrPassive(fighterOnTop) == act) {
				generateComment(Comments.inButterflyGuardMoves);
			} else {
				generateComment(Comments.defInButterflyGuardMoves);
			}
			break;
		}
		...
		if (def >= at) {
			...
			doComment(act, pas, extractComment(fullComment));
			// act.increasePoints(Bout.getCurrentRound(), ApplicationUtils.MOVEPOINTS);
			processAfterMovePosition(act, pas, extractFinalSuccessPosition(fullComment));
		}
	}
	...
	public void actSubmission(Fighter act, Fighter pas) {
		...
		if (act.getName().equals(fighterActiveOrPassive(fighterOnTop).getName())) {
			switch (guardType) {
			case 0:
				generateComment(Comments.rearSub1);
				break;
			case 1:
				generateComment(Comments.fullMountSub1);
				break;
			case 2:
			case 3:
				generateComment(Comments.sideMountSub1);
				break;
			case 4:
				generateComment(Comments.openGuardSub1);
				break;
			default:
			...
			switch (guardType) {
			case -1:
				generateComment(Comments.standUpSub1);
				break;
			case 3:
			case 4:
			case 5:
			case 6:
				generateComment(Comments.closedGuardSub1);
				break;
			default:
				actPositioning(act, pas);
				return;
		...
		hitLocation = extractHitLocation(fullComment);
		// Bout.UpdateStatistic(GetFighterNumber(act), StatisticType.stSubmission, 1,
		// 0);
		...
		if (Def >= At) {
			...
			// act.increasePoints(Bout.getCurrentRound(),
			// ApplicationUtils.LOCKINSUBMISSIONPOINTS);
			hitLocation = extractHitLocation(fullComment);

			if (checkSubmission(act, pas, DamageDone, extractKOSubProb(fullComment))) {
				doComment(act, pas, extractComment(fullComment));
				boutFinished = true;
				finishedType = Comments.misc.get(Sim.SUB);
			...
			processAfterMovePosition(act, pas, extractFinalFailurePosition(fullComment));
			// Bout.UpdateStatistic(GetFighterNumber(act), StatisticType.st
		...
		while (lockedSub && !boutFinished) {
			...
			if (def > at || breakProb >= Sim.MAXLOCKINSUBMISSION) {
				...
				if (!boutFinished && checkSubmission(act, pas, damageDone, extractKOSubProb(fullComment))) {
					...
					doComment(act, pas, returnComment(Comments.subRefStoppage));
					boutFinished = true;
					finishedType = Comments.misc.get(Sim.RES_SUB);
					finishMode = Sim.RES_SUB;
					finishedDescription = moveName;
					fighterWinner = fighterNumber(act);
				} else if (!boutFinished) {
					doComment(act, pas, returnComment(Comments.lockingSubmission));
					breakProb++;
	...
	public boolean actSlamOutSubmission(Fighter act, Fighter pas) {
		...
		generateComment(Comments.slamOut);
		...
		// Modifying statistics
		// bout.getStatistics().get(getFighterNumber(act)).setTakedownsAttemps(extractHitsLaunched(fullComment));
		...
		if (def >= at) {
			...
			// act.increasePoints(bout.getCurrentRound(), Sim.SUCCESSFULTAKEDOWNPOINTS);
			...
			// Modifying statistics
			// Bout.Statistics[GetFighterNumber(act)].TakedownsAchieved =
			// ExtractHitsLanded(FullComment);
	...
	public void actStandUp(Fighter act, Fighter pas) {
		generateComment(Comments.standUp);
	...
	public void actFancyPunch(Fighter act, Fighter pas) {
		...
		case 2:
		case 3:
			generateComment(Comments.fancyPunch1);
			break;
		}

		doComment(act, pas, extractInitComment(fullComment));

		// bout.updateStatistic(getFighterNumber(act), StatisticType.PUNCHES,
		// extractHitsLaunched(fullComment), 0);
		...
		if (def >= at) {
			...
			// bout.updateStatistic(getFighterNumber(act), StatisticType.PUNCHES, 0,
			// extractHitsLanded(fullComment));
		}
	}
	...
	public void actFancyKick(Fighter act, Fighter pas) {
		...
		case 2:
		case 3:
			generateComment(Comments.fancyKick1);
			break;
		}

		doComment(act, pas, extractInitComment(fullComment));

		// bout.updateStatistic(getFighterNumber(act), StatisticType.KICKS,
		// extractHitsLaunched(fullComment), 0);
		...
		if (def >= at) {
			...
			// bout.updateStatistic(getFighterNumber(act), StatisticType.KICKS, 0,
			// extractHitsLanded(fullComment));
	...
	public void actHeadButt(Fighter act, Fighter pas) {
		...
		case 2:
		case 3:
			generateComment(Comments.headbutt);
			break;
		}
		...
		// bout.updateStatistic(getFighterNumber(act), StatType.STAT_CLINCH,
		// extractHitsLaunched(fullComment), 0);
		...
		if (def >= at) {
			...
			// refAwareOfFault(act, pas, Comments.misc.get(Sim.HEADBUTT));

			// Modifying statistics
			// Bout.UpdateStatistic (GetFighterNumber(Act), stClinch, 0,
			// ExtractHitsLanded(FullComment));
		}
	}
	...
	public void actPoke(Fighter act, Fighter pas) {
		...
		if (act.isOnTheGround()) {
			generateComment(Comments.groundEyePoke);
		} else {
			generateComment(Comments.standingEyePoke);
		}
		...
		// Bout.updateStatistic(getFighterNumber(act), stClinch,
		// extractHitsLaunched(fullComment), 0);
		...
		if (def >= at) {
			...
			// refAwareOfFault(act, pas, ApplicationUtils.Misc[ApplicationUtils.EYE_POKE]);
	...
	public void actRest(Fighter act, Fighter pas) {
		...
		if (act.isOnTheGround()) {
			generateComment(Comments.restOnGround);
		} else {
			generateComment(Comments.rest);
		}
		doComment(act, pas, extractInitComment(fullComment));
	...
	public void actGroinKick(Fighter act, Fighter pas) {
		...
		case 2:
		case 3:
			generateComment(Comments.groinKnee);
			break;
		}
		doComment(act, pas, extractInitComment(fullComment));
		hitLocation = extractHitLocation(fullComment);
		// Bout.UpdateStatistic(GetFighterNumber(act), StatType.stClinch,
		// ExtractHitsLaunched(FullComment), 0);
		...
		// RefAwareOfFault(act, pas, ApplicationUtils.Misc[ApplicationUtils.GOIN_KICK]);

		// Bout.UpdateStatistic(GetFighterNumber(act), StatType.stClinch, 0,
		// ExtractHitsLanded(FullComment));
	}
	...
	public void actSlam(Fighter act, Fighter pas) {
		...
		// Bout.getStatistics(GetFighterNumber(act)).setTakedownsAttempts(ExtractHitsLaunched(fullComment));
	...
	public static String replaceParams(Fighter act, Fighter pas, String comment, List<String> paramList) {
		...
		comment = comment.replace("%act", act.getName());
		comment = comment.replace("%act", act.getLastName());
		comment = comment.replace("%pas", pas.getName());
		comment = comment.replace("%pas", pas.getLastName());
	 ...
	 * returnComment(Comments.refAware); try { List<String> params = new
